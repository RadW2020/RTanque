class Monger < RTanque::Bot::Brain
  ...
  attr_reader :arena
	...
	def movimiento
		if self.sensors.ticks.to_s.split('')[-2].to_i.between?(0,3)
			n = -1 
		elsif  self.sensors.ticks.to_s.split('')[-2].to_i.between?(3,6)
			n = 0
		else
			n = 1
		end
		self.command.heading = RTanque::Heading.new_between_points(mi_posicion, destino) + (n * ( RTanque::Heading::ONE_DEGREE * 45))
	end
